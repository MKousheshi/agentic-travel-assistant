...
def _weather_location(
    ...
    """Build a weather-service location payload from an airport row."""
    coordinates = _parse_coordinates(airport.coordinates)
    city = _localized(airport.city, lang)
    ...
    return {
        ...
        "longitude": coordinates["longitude"],
        "timezone": airport.timezone,
        "label": (f"{city} ({airport.airport_code})" if city else airport.airport_code),
    }


def resolve_weather_location_by_airport(
    session: Session,
    airport_code: str,
    *,
    lang: str = "en",
) -> dict[str, Any]:
    """Resolve the weather location for a single airport."""
    airport = session.get(Airport, airport_code)

    if airport is None:
        raise ValueError(f"Airport with airport_code '{airport_code}' not found.")

    return _weather_location(airport, lang)


def resolve_flight_origin_destination(
    session: Session,
    flight_id: int,
    *,
    lang: str = "en",
) -> dict[str, Any]:
    """Resolve origin and destination weather locations for a flight."""
    flight = session.get(Flight, flight_id)

    if flight is None:
        raise ValueError(f"Flight with flight_id '{flight_id}' not found.")

    origin = session.get(Airport, flight.departure_airport)
    destination = session.get(Airport, flight.arrival_airport)

    return {
        "flight_id": flight.flight_id,
        "flight_no": flight.flight_no,
        "origin": _weather_location(origin, lang) if origin else None,
        "destination": (_weather_location(destination, lang) if destination else None),
    }
